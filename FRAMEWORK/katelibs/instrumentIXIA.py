...
class InstrumentIXIA(Equipment):
    ...
    def connect_ixnetwork(self):       ### krepo added ###
        ...
        try:
            ...
            self.__IXN = None
            return self.__ret_func(False,"error", "Bridge connection failed: check ixNetwork server[{}] port[{}] [{}]".format(self.__ixNetworkServerAddress,self.__ixNetworkServerPort,excMsg))
    ...
    def create_all_vports(self, vportlist, vportsMediaType="fiber"):       ### krepo added ###
        ...
        for localVportId in vportlist:
            self.__lc_msg("Vport [{}] creation ".format(localVportId))
            retcode = self.add_single_vport(localVportId, mediaType=vportsMediaType)
            self.__lc_msg("Vport [{}] creation ".format(retcode))
    ...
    def create_traffic(self,
        ...
        if VLanId != 0:
            try:
                self.__lc_msg("[ * * * VLAN CREATION SECTION - BEGIN * * * ]" )
                ...
                self.__lc_msg("ti1ConfEl [{}]".format(ti1ConfEl))
                ...
                self.__lc_msg("ethernetStack [{}]".format(ethernetStack))
                ...
                #self.__lc_msg("protocolTemplateList [{}]".format(protocolTemplateList))
                self.__lc_msg("protocolTemplateList  " )
                for temp in protocolTemplateList:
                    self.__lc_msg("[{}]".format(temp))
                protocolTemplateVLAN = [s for s in protocolTemplateList if "vlan" in s][0]
                self.__lc_msg("protocolTemplateVLAN [{}]".format(protocolTemplateVLAN))
                ...
                self.__lc_msg("destMacField [{}]".format(destMacField))
                self.__IXN.setAttribute(destMacField, '-singleValue', VLanDestMacAddr)
                self.__IXN.commit()
                self.__lc_msg("[ * * * VLAN CREATION SECTION - END * * * ]" )
            except Exception as excMsg:
                self.__IXN = None
                return self.__ret_func(False,"error", "[{}] exception [{}]".format(methodLocalName,excMsg))
        else:
            self.__lc_msg("VLan Creation SKIPPED [{}]")
    ...
    def stop_traffic(self,timeToStop=0):
        methodLocalName = self.__lc_current_method_name(embedKrepoInit=True)
        #self.__lc_msg("Stop all traffic ")
    ...
    def get_port_statistic(self, ipChassis, cardSlot, portNumber):
        ...
        self.__lc_msg("Statistic 2 Retrieve [{}]".format(StatisticToRetrieve))
        try:
            portStats_1 = self.__IXN.getFilteredList(self.__ROOT+'statistics', 'view', '-caption','Port Statistics')[0]
        except Exception as excMsg:
            retDictionary=dict()
